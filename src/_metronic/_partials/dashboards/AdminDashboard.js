...
const AdminDashboard = (props) => {
  ...
  const UpdateCallStat = (data) => {
    ...
    setCallStat({
      totalKeyword:
        data
          ?.map((item) => item.totalKeyword)
          .reduce((prev, next) => prev + next) ?? 0,
      markedKeyword:
        data
          ?.map((item) => item.markedKeywordCount)
          .reduce((prev, next) => prev + next) ?? 0,
      totalCalls: data?.length ?? 0,
      unMarkedKeyword:
        data
          ?.map((item) => item.unMarkedKeywordCount)
          .reduce((prev, next) => prev + next) ?? 0,
      percentage: 0,
    });
  ...
  const onClickSearch = () => {
    if (Utils.isNullOrUndefined(fromDate) || Utils.isNullOrUndefined(toDate)) {
      Utils.showSuccessAlertAutoClose(
        "Please select valid To Date & From Date.",
        "warning"
      );
      return;
    }
    ...
    CRUD.postRequest(
      CRUD.SalesPingAPIRoutes.PhoneCall_GetPhoneCallLog,
      {
        instanceId: Utils.GetCurrentUser().fk_InstanceId,
        agentId,
        fromDate,
        toDate,
      },
  ...
  useEffect(() => {
    // //debugger;
    // CRUD.postRequest(CRUD.SalesPingAPIRoutes.User_GetSystemSpecificUsers, {
    //   roleId: Utils.UserRole.Agent,
    //   instanceId: Utils.GetCurrentUser().fk_InstanceId
    // }, function (response) {
    //   if (response?.data?.success && response.data.result.length > 0) {
          ...
          <div className="card card-custom card-stretch">
            <div className="card-header">
              <div className="card-title">
                <h2 className="card-label">Sales</h2>
              </div>
            </div>
...
{/* dropdown ends */}
              ...
              >
                <thead className="bg-custom text-white">
                  <tr>
                    <th>
                      <i className="fa fa-mobile-alt mr-1 text-center"></i>{" "}
                      Phone No.
                    </th>
                    <th>
                      <i className="fa fa-calendar-alt mr-1"></i> Call Date Time
                    </th>
                    <th>
                      <i className="fa fa-clock mr-1 text-center"></i> Duration
                    </th>
                    <th>
                      <i className="fa fa-thumbs-up mr-1 text-center"></i>{" "}
                      Feedback Positive?
                    </th>
                    <th>
                      <i className="fa fa-pencil mr-1 text-center"></i> Total
                      Words
                    </th>
                    <th>
                      <i className="fa fa-pencil mr-1 text-center"></i> Marked
                      Words
                    </th>
                    <th>
                      <i className="fa fa-pencil mr-1 text-center"></i> Unmarked
                      Words
                    </th>
                  </tr>
                </thead>
                    ...
                    return (
                      <tr key={idx}>
                        ...
                        <td className="text-center">
                          {moment(log.createdOn).format("YYYY-MM-DD HH:mm:ss")}
                        </td>
                        <td className="text-center">
                          {log.durationTime ?? ""}
                        </td>
                        <td className="text-center">
                          {log.isThumbsUp ? "Yes" : "No"}
                        </td>
                        <td className="text-center">
